...
EXTERNAL_PATH = (
    '/Users/javadshabani/Desktop/Projects/IBM/20200928_NYU_XLD8B/in16_out2'
)
...
for resonance_folder in resonance_folders:
    ...
    for root, directory, files in os.walk(data_path):
        for f in files:
            if f.endswith('.csv'):
                sample_info = re.findall(r'(.*?)GHz_averaging_on_(.*?)dB_2001pts.csv',f)
                print(f,sample_info)
...
results = {}
try:
    with open(os.path.join(EXTERNAL_PATH,'pickle_jar','ibm_fit_results.pickle'),'rb') as f:

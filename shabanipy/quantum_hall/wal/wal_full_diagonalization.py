...
def compute_wal_conductance_difference(field, dephasing_field,
    ...
    # Compute the j=0 term of the sum in eq 24
    index = np.tile(np.arange(0, matrix_truncation), (len(field), 1)).T
    j0_sum = np.sum(1.0 / (index + 0.5 + dephasing_field / field), axis=0)

    # Compute the j=1 term of the sum in eq 24
    ...
    j1_sum = np.sum(1.0 / eigenvalues, axis=1)

    # subtract the reference field value from eq 24
    # add the log term from eq 27 to make sure the finite sum would not diverge
    sigma = (j0_sum[1:] - j1_sum[1:]) - (j0_sum[0] - j1_sum[0]) - 2*np.log(field[0]/field[1:])
    return sigma
